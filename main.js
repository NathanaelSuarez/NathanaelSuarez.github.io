class Recommender {
  ...
  loadSettings() {
    ...
    document.getElementById('trainingValue').textContent =
      this.trainingInterval;
  }
  ...
  trainModel() {
    ...
    this.model.params = params.map((p, i) => ({
      coeff: p,
      feature: features[i],
    }));
  ...
  scoreImage(embedding) {
    return this.model.params.reduce(
      (sum, param) => sum + param.coeff * embedding[param.feature],
      0
    );
  }
  ...
  showNextSet(skip = false) {
    ...
    // Always use AI-based candidates if the model is trained, even when skipping
    if (
      this.history.length >= this.trainingInterval &&
      this.model.params.length > 0
    ) {
  ...
  updateProgress() {
    const progress =
      ((this.history.length % this.trainingInterval) / this.trainingInterval) *
      100;
  ...
  updateCounter() {
    const remaining =
      this.trainingInterval - (this.history.length % this.trainingInterval);
    document.getElementById(
      'counter'
    ).textContent = `${remaining}/${this.trainingInterval}`;
  }
...
document
  ...
  .addEventListener('click', () => recommender.showNextSet(true));
